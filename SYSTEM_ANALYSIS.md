...
- Event & Informasi Kampus (Admin CRUD, daftar, detail, pencarian/filter)
...
5. **Event & Informasi Kampus:** Informasi kegiatan, event, dan pengumuman mahasiswa yang dikelola Admin.

6. **Chat:** Komunikasi real-time 1-on-1 antar mahasiswa.

7. **Interaction:** Sistem Rating, Favorite (Bookmark), dan Report (Pelaporan pelanggaran).

8. **Admin & Notification:** Dashboard statistik, moderasi konten, notifikasi event/sistem.

9. **AI:** Asisten pencocokan L&F dan rekomendasi listing.
...
**### 6.3 Event & Informasi Kampus (FR-EVENT)

FR-EVENT-01: Admin dapat membuat, mengubah, dan menghapus informasi event/pengumuman.

FR-EVENT-02: Mahasiswa dapat melihat daftar dan detail event/informasi kampus.

FR-EVENT-03: Mahasiswa dapat mencari dan memfilter event/informasi berdasarkan kategori dan waktu.

FR-EVENT-04: Sistem dapat menampilkan status publikasi informasi (Draft, Published, Archived).

6.4 Interaksi & Chat (FR-INT)
...
### 6.5 Moderasi, Notifikasi & AI (FR-SYS)
...
### 6.6 Student Web & Admin Web Pages

- Student Web: /events menampilkan daftar event/informasi kampus dengan pencarian dan filter; /events/{slug} menampilkan detail event/informasi.
- Admin Web: /events menampilkan daftar event/informasi; /events/create untuk membuat; /events/{id}/edit untuk mengubah; serta aksi publish, archive, dan delete.
...
- `categories`: id, name, slug, type (marketplace/service/event).
...
- `events`: admin_id, category_id, title, slug, description, location, start_at, end_at, status, published_at, dll.
...
**### 11.4 Event & Informasi Kampus (REST)

GET /api/v1/events: Daftar event/informasi dengan pencarian, filter, dan pagination.

POST /api/v1/admin/events: Admin membuat event/informasi.

GET /api/v1/events/{slug}: Detail event/informasi.

PUT, DELETE, PATCH /api/v1/admin/events/{id}/status: Admin mengelola dan mempublikasikan event/informasi.

11.5 Polymorphic Interactions (REST)**
...
### 11.6 Chat & Notification (REST & WS)
